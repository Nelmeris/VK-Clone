...
class GroupsUITableViewController: UITableViewController, UISearchResultsUpdating {
  ...
  override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    cell.setGroupPhoto(group, tableView, indexPath)
  ...
  override func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
    let deleteAction = UITableViewRowAction(style: .default, title: "Покинуть") { [weak self] (action, indexPath) in
      ...
      action = UIAlertAction(title: "Покинуть", style: .destructive) { action in
        VKService.shared.request(method: "groups.leave", parameters: ["group_id": String(strongSelf.groups[indexPath.row].id)]) { _ in
          VKService.shared.getGroups { data in
            RealmService.updateData(data)
          }
        }
